...
export default function BookCarousel({
  ...
  const handleCategoryMouseDown = (e) => {
    ...
    // Chỉ set dragging sau khi thật sự di chuyển (tránh xung đột với click)
    isDragging.current = false;
  };

  const handleCategoryMouseMove = React.useCallback((e) => {
    if (!categoryScrollRef.current) return;
    
    // Nếu di chuyển quá 5px thì coi là drag
    if (!isDragging.current && Math.abs(e.pageX - dragStartX.current) > 5) {
  ...
  const handleCategoryMouseUp = React.useCallback(() => {
    if (!categoryScrollRef.current) return;
    isDragging.current = false;
  ...
  return (
    <div className="space-y-6 relative">
      ...
      {/* Categories - Có thể kéo và ẩn scrollbar */}
      {hasCategories && categories && (
        <div 
          ref={categoryScrollRef}
          className="flex space-x-2 overflow-x-auto pb-2 cursor-grab scrollbar-hide"
          style={{
        ...
        >
          ...
          {categories.map((category, index) => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors select-none ${
        ...
        >
          {(filteredBooks || []).length === 0 ? (
            ...
            filteredBooks.map((book) => {
              // Kiểm tra promotion và tính toán shouldShow
              const hasPromotion = Boolean(book.hasPromotion);
              const hasDiscount = book.discountValue !== null && 
                                 book.discountValue !== undefined && 
                                 Number(book.discountValue) > 0;
              const shouldShowBadge = hasPromotion && hasDiscount;
              
              // Debug: Log promotion data cho từng book
              if (hasPromotion || hasDiscount) {
                console.log("BookCarousel - Book with promotion:", {
                  id: book.id,
                  title: book.title,
                  hasPromotion: book.hasPromotion,
                  discountValue: book.discountValue,
                  shouldShowBadge,
                  computedHasPromotion: hasPromotion,
                  computedHasDiscount: hasDiscount,
                  willRender: shouldShowBadge
                });
                
                // Force render test - nếu shouldShowBadge là true nhưng không hiển thị
                if (shouldShowBadge) {
                  console.log(`✅ Should render badge for book ${book.id}: "${book.title}" with discount ${book.discountValue}%`);
                }
              }
                ...
                >
                  <div className="w-56">
                    ...
                    <div className="relative mb-3" style={{ position: 'relative' }}>
                      ...
                      />
                      <div className="absolute inset-0 bg-black/20 group-hover:bg-black/10 transition-colors rounded-lg" style={{ zIndex: 1 }}></div>
                      
                      {/* Promotion Badge - Góc phải trên, màu đỏ */}
                      {shouldShowBadge ? (
                        <div 
                          style={{ 
                            position: 'absolute',
                        ...
                        >
                          ...
                          >
                            -{Number(book.discountValue).toFixed(0)}%
                          </div>
                        </div>
                      ) : null}
                    ...
                    {book.completionStatus && (
                      <div className="absolute top-3 left-3 z-20">
                        <span className={`px-2 py-1 text-xs text-white rounded-full ${getCompletionStatusColor(book.completionStatus)} shadow-lg block`}>
